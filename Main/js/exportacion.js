...
export async function exportarKPIs(kpis, tendencia) {
  const JsPDF = await cargarJsPDF();
  const doc   = new JsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });

  let y = cabecera(doc, 'Resumen estadístico');

  // Calcular derivados desde el resumen raw del RPC
  ...
  doc.save(`estadisticas_${_fechaArchivo()}.pdf`);
}
